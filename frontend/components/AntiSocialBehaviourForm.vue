<template>
  <div class="pt-0 container mx-auto">
    ...
    <form ref="form" @submit.prevent="submitForm()">
      <!-- <p class="lg:text-[18px] xxxs:text-[15px] text-custom-color font-regular-custom lg:mt-3 lg:mb-3 xxxs:mt-1 xxxs:mb-1">
        Select one
      </p>
      <Dropdown v-model="selectedOption" inputId="dd-city" :options="props.selectOption"
        optionLabel="attributes.title" placeholder="Select..." class="w-full lg:max-w-[632px] min-h-[50px]"/> -->

      <p class="lg:text-[18px] xxxs:text-[16px] text-[#20407C] font-black-custom mt-5">
      ...
      <div class="pt-11 pb-24">
        ...
        <div class="w-full">
          ...
          <InputText type="text" required v-model="contact_number" class="my-4 w-full max-w-[632px] min-h-[50px]" />
          <!-- <Button
            link="contact-submitted"
            value="Submit"
            addClass="my-4 w-full max-w-[632px] min-h-[50px] font-xbold-custom bg-[#2A8CFB] hover:bg-[#1F73D3]  focus:bg-[#175EB1] mt-7 rounded-xs text-white"
          ></Button> -->
...
const onBlurEvent = (e) => {
  if (!onBlurTriggered.value && message.value !== '') {
    logEvent(analytics, 'anti_socail_form_start', {
...
async function sendEmail() {
  try {
    ...
    fileInputs?.value?.forEach((file, index) => {
      if (file) {
        formDataFile.append("form", 'anti-social-behaviour');
   ...
   await axios.post('https://prod-backend.stefphilips.com/api/upload-forms/s3', formDataFile, {
      ...
      .then(async response => {
        ...
        var html =
          ...
          full_name.value +
          "<br/>" +
          // "<strong>Option selected for issue:</strong> Kitchen" +
          // "<br/>" +
        ...
        const formData = {
          data: {
            ...
            email: email.value,
            to: "allocations@stefphilips.com",
        ...
        if (responseData) {
          logEvent(analytics, 'anti_socail_form', {
            custom_param: 'custom_value'
          });
        }
    ...
    await API.post(`api/anti-social-behaviour-forms`, formData);
  } catch (error) {
